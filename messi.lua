...
ElementsTable.ProgressBar = (function()
	...
	function Element:New(Idx, Config)
		...
		assert(Config.Value, "ProgressBar - Missing default value.")
		assert(Config.Min, "ProgressBar - Missing min value.")
		assert(Config.Max, "ProgressBar - Missing max value.")
		...
		local ProgressBarFrame = Components.Element(Config.Title, Config.Description, self.Container, false, Config)
		ProgressBar.SetTitle = ProgressBarFrame.SetTitle
		ProgressBar.SetDesc = ProgressBarFrame.SetDesc

		local ProgressBarInner = New("Frame", {
			Size = UDim2.new(1, 0, 0, 4),
			AnchorPoint = Vector2.new(1, 0.5),
			Position = UDim2.new(1, -10, 0.5, 0),
			BackgroundTransparency = 0.4,
			Parent = ProgressBarFrame.Frame,
			ThemeTag = {
				BackgroundColor3 = "SliderRail",
			},
		}, {
		...
		function ProgressBar:SetValue(Value)
			...
			self.Value = Value
			ProgressFill.Size = UDim2.new((Value - self.Min) / (self.Max - self.Min), 0, 1, 0)
			ProgressLabel.Text = math.floor((Value / self.Max) * 100) .. "%"
			Library:SafeCallback(self.Callback, self.Value)
		end

		ProgressBar:SetValue(Config.Value)

		Library.Options[Idx] = ProgressBar
		return ProgressBar
	end
